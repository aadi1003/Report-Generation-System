...
pipeline {
    ...
    environment {
        REGISTRY = 'ghcr.io'
        IMAGE_NAME = 'aadi1003/report-generation-system'
        // Fixed: Use env.BUILD_ID instead of BUILD_NUMBER directly
    ...
    stages {
        // STAGE 1: CHECKOUT (separate from Docker build)
        stage('Checkout') {
            steps {
                checkout scm
            }
                ...
                script {
                    ...
                    withCredentials([usernamePassword(
                        ...
                        sh '''
                            ...
                            # Tag and push
                            docker tag report-system ghcr.io/${IMAGE_NAME}:latest
                            docker tag report-system ghcr.io/${IMAGE_NAME}:${BUILD_ID}
                            
                            docker push ghcr.io/${IMAGE_NAME}:latest
                            docker push ghcr.io/${IMAGE_NAME}:${BUILD_ID}
                ...
                script {
                    ...
                    withCredentials([usernamePassword(
                        ...
                        sh '''
                            ...
                            # Using npx to run local gh-pages
                            echo "Deploying to GitHub Pages..."
                            npx gh-pages -d dist --dotfiles \
                                --repo https://${GITHUB_USER}:${GITHUB_TOKEN}@github.com/aadi1003/Report-Generation-System.git \
                                --user "Jenkins CI <jenkins@ci>" \
                                --message "Automated deployment by Jenkins [${BUILD_ID}]"
